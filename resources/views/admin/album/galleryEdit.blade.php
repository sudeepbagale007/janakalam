    ...
    <div class="col-md-4">
        <div class="box">
            <div class="box-header with-border">{{ $page_header }}
                <span class="pull-right"><a href="{{ route('albumGallery',$list->id) }}" class="btn btn-warning">{!! VIEWLIST_ICON !!}</a></span>
            </div>
            <div class="box-body">
                <form class="" method="POST" action="{{ route('albumGalleryUpdate',$record->id) }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label for="title">Title <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" value="{{ $record->title }}" id="title" name="title">
                        @if ($errors->has('title'))
